...
def update_menu(db: Session, menu: schemas.MenuUpdate, menu_id: int):
...
def get_all_submenus(db: Session, menu: schemas.MenuOut) -> list[models.Submenu]:
...
def get_submenu(db: Session, menu_id: int, submenu_id: int):
    return db.query(models.Submenu).filter(models.Submenu.parent_menu_id == int(menu_id), \
    models.Submenu.id == int(submenu_id)).first()


def get_submenu_by_title(db: Session, menu_id: int, title: str):
    need_menu = db.query(models.Menu).filter(models.Menu.id == menu_id).first()
    return db.query(need_menu.submenus).filter(need_menu.submenus.title == title).first()


def create_submenu(db: Session, submenu: schemas.SubmenuCreate):
    db_submenu = models.Submenu(title=submenu.title, description=submenu.description, menu_id=submenu.parent_menu_id)
    db.add(db_submenu)
    db.commit()
...
#     db.query(models.Submenu).filter(models.Submenu.parent_menu_id == menu_id, \
#     models.Submenu.id == submenu_id).delete()
#     db.commit()
#     return
...
#     return db.query(models.Dish).filter(models.Dish.suubmenu_id == submenu_id, \
