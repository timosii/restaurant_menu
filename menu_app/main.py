...
def delete_submenu(menu_id: UUID, 
                   submenu_id: UUID, 
                   db: Session = Depends(get_db)):
    crud.delete_submenu(menu_id=menu_id, 
...
def get_dishes(menu_id: UUID, 
               submenu_id: UUID, 
               db: Session = Depends(get_db)):
    dishes = crud.get_dishes(submenu_id=submenu_id, db=db)
    for dish in dishes:
...
def receive_current_dish(dish_id: UUID, 
    ...
    current_dish = crud.get_dish(dish_id=dish_id, db=db)
...
def create_dish(menu_id: UUID, 
                ...
                dish: schemas.DishIn, 
                db: Session = Depends(get_db)):
    current_dish = crud.create_dish(menu_id=menu_id, 
                                    submenu_id=submenu_id, 
                                    dish=dish, 
                                    db=db)
    current_dish.price = str(current_dish.price)
...
def update_current_menu(menu_id: UUID, 
                        ...
                        dish: schemas.DishIn, 
                        db: Session = Depends(get_db)):
    dish_to_update = crud.update_dish(menu_id=menu_id, 
                                      submenu_id=submenu_id, 
                                      dish_id=dish_id, 
                                      dish=dish, 
                                      db=db)
...
def delete_dish(menu_id: UUID, 
                ...
                dish_id: UUID, 
                db: Session = Depends(get_db)):
    crud.delete_dish(menu_id=menu_id, 
                     submenu_id=submenu_id, 
                     dish_id=dish_id, 
                     db=db)
    return JSONResponse(status_code=200, 
